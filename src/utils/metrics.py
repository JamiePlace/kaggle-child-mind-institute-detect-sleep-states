...
def score(
    ...
    assert set(tolerances.keys()) == set(
        solution[event_column_name]
...
def event_detection_ap(
    ...
    ground_truths = solution.query(
        "event not in ['start', 'end']"
    ...
    detections_grouped = aggregation_keys.merge(
        detections_filtered,
        on=[event_column_name, series_id_column_name],
        how="left",
    ...
    mean_ap = ap_table.groupby(event_column_name).mean().sum() / len(
        event_classes
    )
...
def match_detections(
    ...
    is_matched = np.full_like(
        detections_sorted[event_column_name], False, dtype=bool
    )
